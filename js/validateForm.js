...
function addAdmin() {
  ...
  if(!validateName(username.value, "name_error"))
    username.focus();
  else if(!notNull(password.value, "password error"))
    password.focus();
  else {
    var xhttp = new XMLHttpRequest();
  	...
  	xhttp.open("GET", "php/server.php?name=" + username.value + "&password=" + password.value, true);
  	xhttp.send();
  }
  return false;
}
